...
/** A service that synchronizes Twask with Remember the Milk
 * @author Andrew Xu
 * @author timsu
 *
 */
public abstract class SyncService {
    ...
    void synchronizeService(final Activity activity) {
        ...
        SyncService.progressDialog = new ProgressDialog(activity);
    ...
    protected interface SynchronizeHelper {
         ...
         */
        void deleteTask(SyncMappingModel mapping) throws IOException;
    }
    ...
    protected void synchronizeTasks(final Activity activity, LinkedList<TaskProxy>
        ...
        SyncMappingController syncController = Synchronizer.getSyncController(activity);
        ...
        for(TaskIdentifier taskId : data.newlyCreatedTasks) {
            ...
            if(data.newRemoteTasks.containsKey(task.getName())) {
                ...
                SyncMappingModel mapping = new SyncMappingModel(taskId, getId(),
            ...
            SyncMappingModel mapping = new SyncMappingModel(taskId, getId(), remoteId);
            syncController.saveSyncMapping(mapping);
            data.localIdToSyncMapping.put(taskId, mapping);
        ...
        for(TaskIdentifier taskId : data.deletedTasks) {
            SyncMappingModel mapping = data.localIdToSyncMapping.get(taskId);
        ...
        for(SyncMappingModel mapping : data.localChanges) {
        ...
        for(TaskProxy remoteTask : remoteTasks) {
            ...
            if(mapping == null) {
                ...
                if(mapping == null) {
                    try {
                        mapping = new SyncMappingModel(task.getTaskIdentifier(), remoteTask);
                        syncController.saveSyncMapping(mapping);
            ...
            NotificationsReceiver.updateAlarm(activity, taskController, alertController,
                    task);
    ...
    private class SyncData {
        HashSet<SyncMappingModel> mappings;
        HashSet<TaskIdentifier> activeTasks;
        HashSet<TaskIdentifier> allTasks;

        HashMap<String, SyncMappingModel> remoteIdToSyncMapping;
        HashMap<TaskIdentifier, SyncMappingModel> localIdToSyncMapping;

        HashSet<SyncMappingModel> localChanges;
        ...
        public SyncData(Activity activity, LinkedList<TaskProxy> remoteTasks) {
            ...
            remoteIdToSyncMapping = new HashMap<String, SyncMappingModel>();
            localIdToSyncMapping = new HashMap<TaskIdentifier, SyncMappingModel>();
            localChanges = new HashSet<SyncMappingModel>();
            mappedTasks = new HashSet<TaskIdentifier>();
            for(SyncMappingModel mapping : mappings) {
            ...
            for(TaskProxy remoteTask : remoteTasks) {
                if(remoteIdToSyncMapping.containsKey(remoteTask.getRemoteId())) {
                    SyncMappingModel mapping = remoteIdToSyncMapping.get(remoteTask.getRemoteId());
